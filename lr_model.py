...
class KerasModel(object):
    ...
    def createMaster(self, drug_dim, cellline_dim):
        ...
        x_cellline = Conv2D(filters=50, kernel_size=(1, 700), strides=(1, 5), activation='tanh', padding='valid')(
            celline_input)
        x_cellline = MaxPooling2D(pool_size=(1, 5))(x_cellline)
        x_cellline = Conv2D(filters=30, kernel_size=(1, 5), strides=(1, 2), activation='relu', padding='valid')(
            x_cellline)
        ...
        x_drug = Conv2D(filters=50, kernel_size=(1, 200), strides=(1, 3), activation='tanh', padding='valid')(
            drug_input)
    ...
    def createShallow(self, drug_dim, cellline_dim):
        drug_input = Input(shape=(1, drug_dim, 1), name='drug_feat_input')
        celline_input = Input(shape=(1, cellline_dim, 1), name='cellline_feat_input')
        x_cellline = Conv2D(filters=64, kernel_size=(1, 700), strides=(1, 5), activation='tanh', padding='valid')(
            celline_input)
        x_cellline = MaxPooling2D(pool_size=(1, 5))(x_cellline)
        x_cellline = Flatten()(x_cellline)
        x_cellline = Dense(100, activation='relu')(x_cellline)
        ...
        x_drug = Conv2D(filters=64, kernel_size=(1, 200), strides=(1, 3), activation='tanh', padding='valid')(
            drug_input)
        x_drug = MaxPooling2D(pool_size=(1, 5))(x_drug)
        x_drug = Flatten()(x_drug)
        x_drug = Dense(100, activation='relu')(x_drug)
    ...
    def createUnified(self, drug_dim, cellline_dim):
        ...
        x_cellline = Lambda(lambda x: K.expand_dims(x, axis=1))(x_cellline)
        x_cellline = Conv2D(filters=30, kernel_size=(1, 150), strides=(1, 1), activation='relu', padding='valid')(
            x_cellline)
        x_cellline = MaxPooling2D(pool_size=(1, 2))(x_cellline)
        x_cellline = Conv2D(filters=10, kernel_size=(1, 5), strides=(1, 1), activation='relu', padding='valid')(
            x_cellline)
        x_cellline = MaxPooling2D(pool_size=(1, 3))(x_cellline)
        x_cellline = Conv2D(filters=5, kernel_size=(1, 5), strides=(1, 1), activation='relu', padding='valid')(
            x_cellline)
        x_cellline = MaxPooling2D(pool_size=(1, 3))(x_cellline)
...
class KerasMultiSourceGCNModel(object):
    ...
    def createMaster(self, drug_dim, units_list, use_relu=True, use_bn=True, use_GMP=True):
        # drug_input = Input(shape=(1,drug_dim,1),name='drug_feat_input')
        drug_feat_input1 = Input(shape=(None, drug_dim), name='drug_feat_input1')  # drug_dim=75
        ...
        if use_relu:
            ...
            GCN_layer1 = Activation('tanh')(GCN_layer1)
        if use_bn:
            GCN_layer1 = BatchNormalization()(GCN_layer1)
        GCN_layer1 = Dropout(0.1)(GCN_layer1)

        for i in range(len(units_list) - 1):
            GCN_layer1 = GraphConv(units=units_list[i + 1], step_num=1)([GCN_layer1, drug_adj_input1])
            if use_relu:
                GCN_layer1 = Activation('relu')(GCN_layer1)
            else:
                GCN_layer1 = Activation('tanh')(GCN_layer1)
        ...
        GCN_layer1 = GraphConv(units=100, step_num=1)([GCN_layer1, drug_adj_input1])
        if use_relu:
            GCN_layer1 = Activation('relu')(GCN_layer1)
        else:
            GCN_layer1 = Activation('tanh')(GCN_layer1)
        ...
        # global pooling
        if use_GMP:
            x_drug1 = GlobalMaxPooling1D()(GCN_layer1)
        else:
            x_drug1 = GlobalAveragePooling1D()(GCN_layer1)

        drug_feat_input2 = Input(shape=(None, drug_dim), name='drug_feat_input2')  # drug_dim=75
        drug_adj_input2 = Input(shape=(None, None), name='drug_adj_input2')

        # drug feature with GCN1
        GCN_layer2 = GraphConv(units=units_list[0], step_num=1)([drug_feat_input2, drug_adj_input2])
        if use_relu:
            GCN_layer2 = Activation('relu')(GCN_layer2)
        else:
            GCN_layer2 = Activation('tanh')(GCN_layer2)
        if use_bn:
            GCN_layer2 = BatchNormalization()(GCN_layer2)
        GCN_layer2 = Dropout(0.1)(GCN_layer2)

        for i in range(len(units_list) - 1):
            GCN_layer2 = GraphConv(units=units_list[i + 1], step_num=1)([GCN_layer2, drug_adj_input2])
            if use_relu:
                GCN_layer2 = Activation('relu')(GCN_layer2)
            else:
                GCN_layer2 = Activation('tanh')(GCN_layer2)
        ...
        GCN_layer2 = GraphConv(units=100, step_num=1)([GCN_layer2, drug_adj_input2])
        if use_relu:
            GCN_layer2 = Activation('relu')(GCN_layer2)
        else:
            GCN_layer2 = Activation('tanh')(GCN_layer2)
        ...
        # global pooling
        if use_GMP:
            x_drug2 = GlobalMaxPooling1D()(GCN_layer2)
        else:
            x_drug2 = GlobalAveragePooling1D()(GCN_layer2)
        ...
        # x = Lambda(lambda x: K.expand_dims(x, axis=-1))(x)
        # x = Lambda(lambda x: K.expand_dims(x, axis=1))(x)
        # print('after lambda x shape is %s' % x.shape)  # (?, 1, 300, 1)
        # x = Conv2D(filters=30, kernel_size=(1, 150), strides=(1, 1), activation='relu', padding='valid')(x)
        # print('after first conv x shape is %s' % x.shape)  # (?, 1, 151, 30)
        # x = MaxPooling2D(pool_size=(1, 2))(x)
        # # (?, 1, 75, 30)
        # x = Conv2D(filters=10, kernel_size=(1, 5), strides=(1, 1), activation='relu', padding='valid')(x)
        # x = MaxPooling2D(pool_size=(1, 2))(x)
        ...
        # x = Conv2D(filters=10, kernel_size=(1, 5), strides=(1, 1), activation='relu', padding='valid')(x)
        # print('just after second x shape is %s ' % x.shape)  # (1, 71, 10)
        # x = MaxPooling2D(pool_size=(1, 3))(x)
        # print('after second conv x shape is %s' % x.shape)  # (?, 1, 23, 10)
        # x = Conv2D(filters=5, kernel_size=(1, 5), strides=(1, 1), activation='relu', padding='valid')(x)
        # x = MaxPooling2D(pool_size=(1, 3))(x)
        # print('after third conv x shape is %s' % x.shape)  # (?, 1, 6, 5)
        # # sys.exit()
        # x = Dropout(0.1)(x)
        # x = Flatten()(x)
        # x = Dropout(0.2)(x)
        ...
        model = Model(inputs=[drug_feat_input1, drug_adj_input1, drug_feat_input2, drug_adj_input2], outputs=output)
        return model

    def createNoAdj(self, drug_dim, units_list, use_relu=True, use_bn=True, use_GMP=True):
        # drug_input = Input(shape=(1,drug_dim,1),name='drug_feat_input')
        drug_feat_input1 = Input(shape=(None, drug_dim), name='drug_feat_input1')  # drug_dim=75
        drug_adj_input1 = Input(shape=(None, None), name='drug_adj_input1')

        drug_feat_input2 = Input(shape=(None, drug_dim), name='drug_feat_input2')
        drug_adj_input2 = Input(shape=(None, None), name='drug_adj_input2')

        # drug feature with Dense
        fc_layer1 = Dense(units_list[0])(drug_feat_input1)

        if use_relu:
            fc_layer1 = Activation('relu')(fc_layer1)
        else:
            fc_layer1 = Activation('tanh')(fc_layer1)

        for i in range(len(units_list) - 1):
            fc_layer1 = Dense(units_list[i + 1])(fc_layer1)
            if use_relu:
                fc_layer1 = Activation('relu')(fc_layer1)
            else:
                fc_layer1 = Activation('tanh')(fc_layer1)

        fc_layer1 = Dense(100)(fc_layer1)
        if use_relu:
            fc_layer1 = Activation('relu')(fc_layer1)
        else:
            fc_layer1 = Activation('tanh')(fc_layer1)
        ...
        # global pooling
        if use_GMP:
            x_drug1 = GlobalMaxPooling1D()(fc_layer1)
        else:
            x_drug1 = GlobalAveragePooling1D()(fc_layer1)

        fc_layer2 = Dense(units_list[0])(drug_feat_input2)

        if use_relu:
            fc_layer2 = Activation('relu')(fc_layer2)
        else:
            fc_layer2 = Activation('tanh')(fc_layer2)

        for i in range(len(units_list) - 1):
            fc_layer2 = Dense(units_list[i + 1])(fc_layer2)
            if use_relu:
                fc_layer2 = Activation('relu')(fc_layer2)
            else:
                fc_layer2 = Activation('tanh')(fc_layer2)

        fc_layer2 = Dense(100)(fc_layer2)
        if use_relu:
            fc_layer2 = Activation('relu')(fc_layer2)
        else:
            fc_layer2 = Activation('tanh')(fc_layer2)
        ...
        # global pooling
        if use_GMP:
            x_drug2 = GlobalMaxPooling1D()(fc_layer2)
        else:
            x_drug2 = GlobalAveragePooling1D()(fc_layer2)
...
class KerasMultiSourceDualGCNModel(object):
    ...
    def createMaster(self, drug_dim, cell_line_dim, units_list, use_relu=True, use_bn=True, use_GMP=True):
        ...
        if self.regr:
            ...
            output = Dense(1, activation='sigmoid', name='output')(x)
        model = Model(inputs=[drug_feat_input, drug_adj_input, cell_line_feat_input, cell_line_adj_input],
                      outputs=output)
        return model
    ...
    def validMaster(self, drug_dim, cell_line_dim, nb_genes, units_list, use_relu=True, use_bn=True, use_GMP=True):
        drug_feat_input = Input(shape=(None, drug_dim), name='drug_feat_input')  # batch*100*75
        drug_adj_input = Input(shape=(None, None), name='drug_adj_input')  # batch*100*100
        cell_line_feat_input = Input(shape=(nb_genes, cell_line_dim),
                                     name='cell_line_feat_input')  # batch*nb_gene*nb_feat
        # drug feature with GCN
        GCN_layer = GraphConv(units=units_list[0], step_num=1)([drug_feat_input, drug_adj_input])
        # GCN_layer = GraphConv(units=64,step_num=1)([drug_feat_input,drug_adj_input])
        if use_relu:
        ...
        x_cell_line = Conv2D(filters=256, kernel_size=(cell_line_dim, 50), strides=(1, 1), activation='relu',
                             padding='valid')(x_cell_line)
        ...
        x_cell_line = Conv2D(filters=256, kernel_size=(1, 20), strides=(1, 1), activation='relu', padding='valid')(
            x_cell_line)
        x_cell_line = BatchNormalization()(x_cell_line)
        x_cell_line = MaxPooling2D(pool_size=(1, 2))(x_cell_line)
        x_cell_line = Dropout(0.2)(x_cell_line)
...
class KerastCNNSModel(object):
    ...
    def createMaster(self, drug_shape, cellline_dim, gexpr_dim, cn_dim, methy_dim, use_relu=True, use_bn=True,
                     use_GMP=True):
        # drug_input = Input(shape=(1,drug_dim,1),name='drug_feat_input')
